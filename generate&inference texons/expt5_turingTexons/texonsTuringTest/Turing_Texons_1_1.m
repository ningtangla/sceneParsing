function Turing_Texons_1_1(filename)
...
%     instructionRect = Screen('Rect',instruction);
    ...
    for trial = 1:pic_num
        ...
        random_list(trial)
        if Type(trial) == 1
            img = imread(['answers/demo',num2str(pic_name(random_list(trial))),'_', filename, 'random.png']); 
        end
...
%         end;
    ...
    Screen('Close',w);
    ShowCursor;
    fid=fopen(['Results\','Turing_Texons_1_1_',filename,'.txt'],'w');
