...
def calTexonsLikelihoodLog(row, texonsObserved, pandasOnlySupportArgNumBiggerTwoRowVectvize):
    ...
    observedTexonsFeatureMeanValue = np.mean(texonsInPartition[feature].values)
    texonsFeatureValueLikelihoodLogOnDiffFeatureMean = [stats.norm.logpdf(observedTexonsFeatureMeanValue, featureMean, featureValueStdVarince) - np.log(len(featureDiscreteMean))  for featureMean in featureDiscreteMean]
...
def main():
    ...
    imageList = [9, 14, 15, 25, 26, 30, 31, 36, 38, 39, 45, 46, 47, 48, 50, 51, 54, 55, 57, 59]
    ...
    imagesInformationDf.to_csv('inference/informationContentWithTexonFeatureMean.csv')
