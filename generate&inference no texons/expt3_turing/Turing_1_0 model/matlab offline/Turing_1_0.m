...
%     instructionRect = Screen('Rect',instruction);
    ...
    for trial = 1:pic_num
        ...
        random_list(trial)
        if Type(trial) == 1
            img = imread(['machine/',num2str(pic_name(random_list(trial))), '.png']); %人
        end
        
        if Type(trial) == 2
            img = imread(['human/',num2str(pic_name(random_list(trial))), '.png']); %机器
        end
        
        img_texture = Screen('MakeTexture',w,img);
        imgRect = Screen('Rect', img_texture) * 0.8;
...
%         end;
    ...
    Screen('Close',w);
    ShowCursor;
    fid=fopen(['Results\','Turing_1_0_',filename,'.txt'],'w');
