...
#        f_all_img = 1
        global GAMMA, ALPHA_BASE
        for z in range(self.img_num):
#            print z
            loggamma = np.random.random()*8.0 - 4.0
            GAMMA = np.e**loggamma
#            print GAMMA
            ALPHA_BASE = np.random.randint(2, 200)
...
#    cv2.waitKey()
    cv2.imwrite('E:/ncrp_infer/largeuniformmachine/machine'+str(MACHINE_NUM) + '/'+str(img_num + 1)+'.png', img_show)
