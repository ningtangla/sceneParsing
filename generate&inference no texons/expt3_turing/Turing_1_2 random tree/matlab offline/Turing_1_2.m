...
%     instructionRect = Screen('Rect',instruction);
    ...
    for trial = 1:pic_num
        ...
        random_list(trial)
        if Type(trial) == 1
            img = imread(['machine/',num2str(pic_name(random_list(trial))), '.png']); 
        end
        
        if Type(trial) == 2
            img = imread(['human/',num2str(pic_name(random_list(trial))), '.png']); 
        end
...
%         end;
    ...
    Screen('Close',w);
    ShowCursor;
    fid=fopen(['Results\','Turing_1_2_',filename,'.txt'],'w');
