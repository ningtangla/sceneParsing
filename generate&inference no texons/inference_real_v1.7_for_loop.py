...
class posterior():
    ...
    def __call__(self):
        ...
        return img_loglikelihood_given_gamma_alpha 
...
def img_likelihood(gamma, alpha):
    global ALPHA_BASE,GAMMA
    GAMMA = gamma
    ALPHA_BASE = alpha
    Posterior_Likelihood = posterior(img_num = IMG_NUM)
    Img_Likelihood = Posterior_Likelihood()
...
def main():
    ...
    p_unnormalized = np.array([img_likelihood(gamma, alpha) for gamma, alpha in zip(np.ravel(GAM), np.ravel(ALP))])
