...
#            cv2.line(image, (int(0.8 * lineCoordinate[0][0]), int(0.8 * lineCoordinate[0][1])), (int(0.8 * lineCoordinate[1][0]), int(0.8 * lineCoordinate[1][1])),
#                    colorSpace[tree.node[currNode]['depth'] - 1], max(2, int(lineMaxWidth * numpy.power(0.5, tree.node[currNode]['depth']))))
#
...
class experimentOfDecision():
    ...
    def __call__(self, trialTotalNum, pathOfExperiment):
        ...
        for imgNum in imgTotal:
            ...
            if typeOfAnswer == 2:
                PathOfAnswer = pathOfExperiment + "randomSample/"
        ...
        timeNow = time.time()
        numpy.savetxt('data/Turing2-2/sub' + str(subjectIndex) + '.txt',numpy.vstack([imageIndexTotal, typeOfAnswerTotal, responseTotal]).T, ['%f', '%f','%f'])
