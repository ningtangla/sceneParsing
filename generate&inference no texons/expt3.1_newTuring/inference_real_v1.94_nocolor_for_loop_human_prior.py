...
class possible_tree_generator():  
    ...
    def __call__(self):
        ...
        img_original = cv2.imread('../scenes/'+str(IMG_LIST[self.img_num])+'.png')
...
#            print z
            ...
            example_Tree = Possible_Tree_List[0]
            leaf_nodes = [ n for n,d in dict(example_Tree.out_degree()).items() if d==0]
            if len(leaf_nodes) != ALL_GUEST_NUM:
                print(len(leaf_nodes), z, '!!!')
...
#        return f_all_img
            if len(posterior_list) >= 5:
                top5_index = np.argsort(posterior_list)[-5:]
                top_posterior_list = np.array(posterior_list)[top5_index] 
            else:
                top5_index = np.array(list(range(len(posterior_list))))
                top_posterior_list =  posterior_list
            posterior_sample_list = map(lambda x: x * 1.0 / np.array(top_posterior_list).sum(), top_posterior_list)
            ...
            conditional_entropy_list = map(lambda x: - x * math.log(x, 2), posterior_list_renormalize)
            ...
            for subjectIndex in SUBJECT_NUM_LIST:
                max_index = np.argmax(np.array(posterior_list)) 
                sampled_index = top5_index[list(np.random.multinomial(1, posterior_sample_list)).index(1)]
                ...
                nx.write_gpickle(sampled_tree, 'humanPrior/'+str(IMG_LIST[z])+'sub'+str(subjectIndex)+'.gpickle')
...
#                        nx.write_gpickle(Possible_Tree_List[i * num_graph_col + j], 'E:/ncrp_infer/possible_tree_infer_alpha'+str(ALPHA_BASE)+'_gamma['+str(GAMMA)+']_img_'+str(z)+'_tree_'+str(i*num_graph_col+j)+'.gpickle')
            ...
            y = posterior_list/np.sum(posterior_list)          
...
#    cv2.waitKey()
    cv2.imwrite('humanPrior/'+str(IMG_LIST[img_num])+'sub'+str(subjectIndex)+'.png', img_show)
...
#    export_IC_CE.to_csv('informationcontent_conditionalentropy_try3.csv')
