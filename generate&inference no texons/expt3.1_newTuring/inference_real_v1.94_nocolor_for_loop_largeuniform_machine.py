...
#            print z
	    ...
	    for subjectIndex in SUBJECT_NUM_LIST:
                ...
                GAMMA = np.e**loggamma
    #            print GAMMA
                ALPHA_BASE = 10**np.random.uniform(-3, 3.01)
    ...
    ##        return f_all_img
                ...
                max_index = np.argmax(np.array(posterior_sample_list)) 
                sampled_index = list(np.random.multinomial(1, posterior_sample_list)).index(1)
                ...
                nx.write_gpickle(sampled_tree, 'nonformativePrior/'+str(IMG_LIST[z])+'sub'+str(subjectIndex)+'.gpickle')
...
#    cv2.waitKey()
    cv2.imwrite('nonformativePrior/'+str(IMG_LIST[img_num])+'sub'+str(subjectIndex)+'.png', img_show)
