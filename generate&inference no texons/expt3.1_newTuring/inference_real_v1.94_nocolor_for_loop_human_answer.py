...
#        return f_all_img
            ...
            humanAnswerData = pd.read_csv('humanAnswerFiles/humanAnswerResult.csv')
            for subjectIndex in SUBJECT_NUM_LIST:
                humanTreeIndex = int(humanAnswerData[(humanAnswerData['subject'] == subjectIndex) & (humanAnswerData['image'] == IMG_LIST[z])]['treeIndex'].values)
                
                sampled_tree = Possible_Tree_List[humanTreeIndex]
                sampled_tree.node[1]['possible_trees_num'] = len(Possible_Tree_List)
                sampled_tree.node[1]['p_poster_list'] = posterior_list
                
                nx.write_gpickle(sampled_tree, 'humanAnswer/'+str(IMG_LIST[z])+'sub'+str(subjectIndex)+'.gpickle')
...
#    cv2.waitKey()
    cv2.imwrite('humanAnswer/'+str(IMG_LIST[img_num])+'sub'+str(subjectIndex)+'.png', img_show)
