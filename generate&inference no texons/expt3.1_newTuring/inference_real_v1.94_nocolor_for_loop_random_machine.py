...
SUBJECT_NUM_LIST = range(1, 17)

#IMG_LIST = range(17001, 19001)
IMG_LIST = np.array([95,409,545,634,695,991,1260,2067,2116,2240,2541,2656,2788,3007,3354,3395,3596,3799,4119,4510]) + 10000
...
class possible_tree_generator():  
    ...
    def __call__(self):
        ...
        img_init = cv2.imread('../scenes/'+str(IMG_LIST[self.img_num])+'.png')
...
#        cv2.destroyAllWindows()
        ...
        tree_init.add_node(1, img = img_init, complete = 0, terminal = 0, x = [1, 1024], y = [1, 768], p_like = 1, p_poster_list = [], guest = [], depth = 1, possible_trees_num = 0)
        ...
        if len(edges_vertical) != 2:
            ...
            for i in range(len(edges_vertical) - 2):
                ...
                for z in range(len(combination_list)):
                    ...
                    tree_temp.add_node(new_node_id_start + 1, img = img_to_cut[0:img_height, edges_vertical[0] + 1:edges_vertical[combination_list[z][0] + 1]], terminal = 0, x = [last_x[0], last_x[0] + edges_vertical[combination_list[z][0] + 1]], y = last_y, cut_proportion = [], guest = [], depth = tree.node[curr_leaf_node]['depth'] + 1)
                    ...
                    tree_temp.add_node(new_node_id_start + len(combination_list[z]) + 1, img = img_to_cut[0:img_height, edges_vertical[combination_list[z][-1] + 1] + 1:edges_vertical[-1]], terminal = 0, x = [last_x[0] + edges_vertical[combination_list[z][-1] + 1], last_x[1]], y = last_y, cut_proportion = [], guest = [], depth = tree.node[curr_leaf_node]['depth'] + 1)
            ...
            for i in range(len(edges_horizontal) - 2):
                ...
                for z in range(len(combination_list)):
                    ...
                    tree_temp.add_node(new_node_id_start + 1, img = img_to_cut[edges_vertical[0] + 1:edges_horizontal[combination_list[z][0] + 1], 0:img_width], terminal = 0, y = [last_y[0], last_y[0] + edges_horizontal[combination_list[z][0] + 1]], x = last_x, cut_proportion = [], guest = [], depth = tree.node[curr_leaf_node]['depth'] + 1)
                    tree_temp.add_edge(curr_leaf_node , new_node_id_start + 1)
                    cut_propotion.append((edges_horizontal[combination_list[z][0] + 1] - (edges_horizontal[0] + 1) + 1)/img_height)
                    if len(combination_list[z]) != 1:
                        for j in range(len(combination_list[z]) - 1):
                            tree_temp.add_node(new_node_id_start + 1 + (j + 1), img = img_to_cut[(edges_horizontal[combination_list[z][j] + 1] + 1):edges_horizontal[combination_list[z][j + 1] + 1], 0:img_width], terminal = 0, y = [last_y[0] + edges_horizontal[combination_list[z][j] + 1], last_y[0] + edges_horizontal[combination_list[z][j + 1] + 1]], x = last_x, cut_proportion = [], guest = [], depth = tree.node[curr_leaf_node]['depth'] + 1)
                            tree_temp.add_edge(curr_leaf_node, new_node_id_start + + 1 + (j + 1))
                            cut_propotion.append((edges_horizontal[combination_list[z][j + 1] + 1] - (edges_horizontal[combination_list[z][j] + 1] + 1) + 1)/img_height)       
                    
                    tree_temp.add_node(new_node_id_start + len(combination_list[z]) + 1, img = img_to_cut[edges_horizontal[combination_list[z][-1] + 1] + 1:edges_horizontal[-1], 0:img_width], terminal = 0, y = [last_y[0] + edges_horizontal[combination_list[z][-1] + 1], last_y[1]], x = last_x, cut_proportion = [], guest = [], depth = tree.node[curr_leaf_node]['depth'] + 1)
                    tree_temp.add_edge(curr_leaf_node , new_node_id_start + len(combination_list[z]) + 1)
                    cut_propotion.append(1 - np.array(cut_propotion).sum())
...
def make_posterior_function(possible_tree):
    g = possible_tree
    nonleaf_nodes = [ n for n,d in dict(g.out_degree()).items() if d!=0]                 
...
#            print z
            ...
            posterior_list = [1/len(Possible_Tree_List)] * len(Possible_Tree_List)
#            f_img_likelihood = reduce(op.add, map(make_posterior_function, Possible_Tree_List))
#            f_all_img = f_all_img * f_img_likelihood
...
#            
            for subjectIndex in SUBJECT_NUM_LIST:
                max_index = np.random.randint(len(Possible_Tree_List))
                sampled_index = np.random.randint(len(Possible_Tree_List))
		...
		most_like_tree = Possible_Tree_List[max_index]
                ...
                sampled_tree = Possible_Tree_List[sampled_index]
                sampled_tree.node[1]['possible_trees_num'] = len(Possible_Tree_List)
                sampled_tree.node[1]['p_poster_list'] = posterior_list

                nx.write_gpickle(sampled_tree, 'randomSample/'+str(IMG_LIST[z])+'sub'+str(subjectIndex)+'.gpickle')
                visualization(sampled_tree, z, subjectIndex)
...
##                        label_list = [str(len(Possible_Tree_List[i * num_graph_col + j].node[n]['guest'])) for n in Possible_Tree_List[i * num_graph_col + j].nodes()]
#                        label_selections = {}                        
#                        leaf_nodes = [ n for n,d in dict(Possible_Tree_List[i * num_graph_col + j].out_degree()).items() if d==0]
...
#                            label_selections[leaf_nodes[t]] = leaf_nodes.index(leaf_nodes[t]) + 1
...
#                        nx.draw_networkx_labels(Possible_Tree_List[i*num_graph_col + j],pos = position, labels = label_selections, font_size=16)
#                        nx.draw_networkx_edges(Possible_Tree_List[i*num_graph_col + j], pos = position)
#                        plt.savefig('treesForScenes/'+str(IMG_LIST[z])+'_'  +str(i * num_graph_col + j)+'.png')
#                        plt.close()
...
#                        
#                        tree_graph_img = cv2.imread('treesForScenes/'+str(IMG_LIST[z])+'_'+str(i * num_graph_col + j)+'.png')
...
#    
#    
#            img_to_infer = cv2.imread('images/'+str(IMG_LIST[z])+'.png')
...
#            plt.savefig('treesForScenes/'+str(IMG_LIST[z])+'_hist.png')   
#            plt.close()
#            img_hist = cv2.imread('treesForScenes/'+str(IMG_LIST[z])+'_hist.png')
...
##            cv2.waitKey()
#            cv2.imwrite('treesForScenes/alpha'+str(ALPHA_BASE)+'_gamma['+str(GAMMA)+']_'+str(IMG_LIST[z])+'.png', img_trees)
#            cv2.destroyAllWindows()
            
            
#            print 'ok!!', most_like_tree.nodes(), most_like_tree.edges()
#            nx.write_gpickle(most_like_tree, 'human_prior/most_like_tree_'+str(IMG_LIST[z])+'.gpickle')
    
def visualization(most_like_tree, img_num, subjectIndex):
#    cv2.namedWindow('inference_demo')
    img_show = np.zeros([IMAGE_HEIGHT, IMAGE_WIDTH + GRAPH_WIDTH, 3], 'uint8')
    img_to_infer = cv2.imread('images/'+str(IMG_LIST[img_num]) +'.png')
    img_to_infer_resize =  cv2.resize(img_to_infer, (IMAGE_WIDTH, int(IMAGE_HEIGHT * 0.5)), interpolation = cv2.INTER_CUBIC)
    img_show[int(IMAGE_HEIGHT * 0.25):int(IMAGE_HEIGHT * 0.75), 20:IMAGE_WIDTH + 20] = img_to_infer_resize
    
    intervel_x_list = [0, GRAPH_WIDTH]
    intervel_y = IMAGE_HEIGHT/(most_like_tree.node[len(most_like_tree.nodes())]['depth'] + 1)
    ...
    for i in range(1, len(most_like_tree.nodes()) + 1):
        ...
        children = list(nx.DiGraph.successors(most_like_tree, i))
...
#                           font_parm, 0.8, (0,  0, 190), 1)
            ...
            cv2.putText(img_show, 
                        str(leaf_node_num), 
                        (int(node_draw_x_list[i] + IMAGE_WIDTH - node_radius/2) , int(intervel_y * depth + node_radius/2)), 
                        font_parm, 0.8, (255,  255, 255), 1)
            
            cv2.putText(img_show, 
                        str(leaf_node_num), 
                        (int(np.sum(most_like_tree.node[i]['x'])/4) + 20 , int(np.sum(most_like_tree.node[i]['y'])/4 + IMAGE_HEIGHT * 0.26)), 
                        font_parm, 1, (255,  255, 255), 1)
#    cv2.imshow('inference_demo', img_show)
#    cv2.waitKey()
    cv2.imwrite('randomSample/'+str(IMG_LIST[img_num])+'sub'+str(subjectIndex)+'.png', img_show)
...
def main():
    global MACHINE_NUM
#    F_Img_Likelihood = Function('F_Img_Likelihood')
#    loggamma_hyper_sample = np.arange(-2, 2.01, 1)
##    gamma_hyper_sample = map(lambda x: np.e**x, loggamma_hyper_sample)
#    gamma_hyper_sample = np.arange(0.1, 2.01, 0.05)
#    logalpha_hyper_sample = np.arange(-4, 4.01, 1)
##    alpha_hyper_sample = map(lambda x: np.e**x, logalpha_hyper_sample)
#    alpha_hyper_sample = np.arange(1.5, 7.01, 0.1)
...
#    est = zip(np.ravel(GAM), np.ravel(ALP))[np.argmax(P)]
#    print est
#    
#    fig = plt.figure()
#    ax = fig.add_subplot(111, projection='3d')
#    ax.plot_surface(GAM, ALP, P)
#    ax.set_xlabel('GAM')
#    ax.set_ylabel('ALP')
#    ax.set_zlabel('P')
#    plt.show()
