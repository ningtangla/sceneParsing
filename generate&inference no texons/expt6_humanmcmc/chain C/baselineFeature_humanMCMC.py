...
class likelihoodBaseOnFeatureValues():
    ...
    def __call__(self):
        ...
        for ii in self.sub_num:
            for jj in self.img_list:
            
                img_num = str(ii) + '_' + str(jj)
                print(img_num)
                ...
                for _ in range(500):
                    ...
                    nodeIndex = np.array([np.arange(0, 5, 1) for _ in range(len(featureValuesAllImg))])
                    map(np.random.shuffle, nodeIndex)

                    heightValuesTrainImg = np.array([heightValuesTrainImgOrigin.values[i][nodeIndex[i]] for i in range(len(featureValuesAllImg))])
                    widthValuesTrainImg = np.array([widthValuesTrainImgOrigin.values[i][nodeIndex[i]] for i in range(len(featureValuesAllImg))])
                    sizeValuesTrainImg = np.array([sizeValuesTrainImgOrigin.values[i][nodeIndex[i]] for i in range(len(featureValuesAllImg))])
                    ...
                    probabilityImg = scipy.stats.multivariate_normal.pdf(partsNormalizedHeights, np.zeros(5), np.ones(5), allow_singular = True) \
                    ...
                    informationContentImgTempList.append(informationContentImg)
                meanInformationContentImg = np.mean(informationContentImgTempList)
                INFORMATION_CONTENT_VALUE_LIST.append(meanInformationContentImg)
def main():           
    ...
    export_IC_CE.to_csv('informationcontent_baseline_feature_expt6_chainC.csv')
    print ('okkkkk')
