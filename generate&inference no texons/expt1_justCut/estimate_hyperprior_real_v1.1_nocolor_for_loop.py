...
class posterior():
    def __init__(self, train_img_index):
        self.train_img_inedx = train_img_index
        
    def __call__(self):
#        img_loglikelihood_given_gamma_alpha = 1
        f_img_likelihood = Function('f_img_likelihood')
        f_all_img = 1
        for z in self.train_img_inedx:
            print z
...
def visualization(most_like_tree, img_num):
    ...
    img_to_infer = cv2.imread('draw/blank1.0/'+IMG_LIST[img_num]+'.png')
...
#    alpha_hyper_sample = map(lambda x: np.e**x, logalpha_hyper_sample)
    ...
    maxLikelihood_list = []
    for folder_index in range(num_K_folder):
        ...
        Posterior_Likelihood = posterior(train_img_index)
        F_Img_Likelihood = Posterior_Likelihood()
        Test_Likelihood = posterior(test_img_index)
        F_Testimg_Likelihood = Test_Likelihood()
        ...
        train_gamma, train_alpha = est
        p_test = float(F_Testimg_Likelihood.subs({'theta': train_gamma, 'alpha': train_alpha}))
        maxLikelihood_list.append(p_test)
        print p_test
        
        heat_map = {'gamma': GAM.flatten(), 'alpha': ALP.flatten(), 'p': P.flatten()}    
        export_IC_CE = pd.DataFrame(heat_map, columns = ['gamma', 'alpha', 'p'])
        export_IC_CE.to_csv('hyperprior_1.1' + str(folder_index) + '.csv')
    
    print(maxLikelihood_list) 
    print(np.mean(maxLikelihood_list))
    export_maxLikelihood = pd.DataFrame(maxLikelihood_list)
    export_maxLikelihood.to_csv('mean_maxLikelihood_1.1.csv')
    #    fig = plt.figure()
