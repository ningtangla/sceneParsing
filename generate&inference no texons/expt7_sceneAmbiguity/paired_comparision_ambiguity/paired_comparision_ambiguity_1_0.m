function paired_comparision_1_0(filename)
...
%     instructionRect = Screen('Rect',instruction);
    ...
    pic_name = [10070,10196,10751,10765,10916,11075,11246,11555,12070,12590,12861,13414,13819,14126,14154,14238,14390,14394,14865,14928];
    temp = 1;
    for i = 1:19
        for j = i + 1:20
            type{temp, 1} = i;
    ...
    for trial = 1:pic_num * (pic_num - 1) / 2
        ...
        img1 = imread(['Stimular/',num2str(pic_name(type{tuple_num, 1})), '.png']); %左侧
        img2 = imread(['Stimular/',num2str(pic_name(type{tuple_num, 2})), '.png']); %右侧
...
%         WaitSecs(0.6+rand*0.4);   %调节trial间间隔时间
    ...
    Screen('Close',w);
    ShowCursor;
    fid=fopen(['Results\','paired_comparision_ambiguity_1_0_',filename,'.txt'],'w');
