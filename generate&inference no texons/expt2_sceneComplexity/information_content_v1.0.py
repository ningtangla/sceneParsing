...
class possible_tree_generator():  
    ...
    def __call__(self):
        img_init = cv2.imread('E:/ncrp_generate/'+str(self.img_num)+'.png')
...
class posterior():
    ...
    def __call__(self):
        for z in range(self.img_num, self.img_num + IMG_NUM_BATCH):
...
#            print Likelihood_Probability[1]
...
#            cv2.putText(img_trees, 
#                        'information_content:' + str(information_content), 
#                        (int(IMAGE_WIDTH + GRAPH_WIDTH/2 - 125), int(IMAGE_HEIGHT/2 - 50)), 
#                        cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.7, (0,  0, 190), 1)
#            
...
#            cv2.putText(img_trees, 
#                        'conditional_entropy:' + str(conditional_entropy), 
#                        (int(IMAGE_WIDTH + GRAPH_WIDTH/2 - 125), int(IMAGE_HEIGHT/2)), 
#                        cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.7, (0,  0, 190), 1)
#            
...
def visualization(most_like_tree, img_num):
    ...
    img_to_infer = cv2.imread('E:/ncrp_generate/'+str(img_num)+'.png')
...
def main():           
    ...
    export_IC_CE.to_csv('E:/pic_IC_and_CE/informationcontent_conditionalentropy_' + str(IMG_NUM) +'_' + str(IMG_NUM_BATCH) + '.csv')
    print 'okkkkk'
