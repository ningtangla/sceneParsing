...
output: all_possible_infer_trees and most_likely_tree
        ...
        if len(edges_vertical) != 2:
            for i in range(len(edges_vertical) - 2):
                ...
                for z in range(len(combination_list)):
                    ...
                    tree_temp.node[1]['p'] = tree_temp.node[1]['p']*cal_p_dirichlet(cut_propotion)*0.5
...
#                    plt.close()
                    nx.write_gpickle(Possible_Tree_List[i * num_graph_col + j], 'E:/ncrp_infer/possible_tree_infer_alpha'+str(ALPHA_BASE)+'_gamma['+str(GAMMA)+']_img_'+str(self.img_num)+'_tree_'+str(i*num_graph_col+j)+'.gpickle')
...
#        cv2.waitKey()
#        cv2.imwrite('E:/ncrp_infer/trees_hist_alpha'+str(ALPHA_BASE)+'_gamma['+str(GAMMA)+']_'+str(self.img_num)+'.png', img_trees)
#        cv2.destroyAllWindows()
        
        
#        print 'ok!!', most_like_tree.nodes(), most_like_tree.edges()
        nx.write_gpickle(most_like_tree, 'E:/ncrp_infer/most_like_tree_alpha'+str(ALPHA_BASE)+'_gamma['+str(GAMMA)+']_'+str(self.img_num)+'.gpickle')
#        g = nx.read_gpickle('E:/ncrp_infer/most_like_tree'+str(ALPHA_BASE)+'_gamma['+str(GAMMA)+']_'+str(self.img_num)+'.gpickle') 
#        visualization(most_like_tree, self.img_num)
        
        self.img_num = self.img_num - 1
        if self.img_num == IMG_NUM - IMG_NUM_BATCH - 1:
